<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<header>
    <div id="logo-box">
        <img id="logo" alt="logo" src="/img/Logo.png" width="200" class="mb20">
        <div>
            <a id="username" class="fon-15 mt20 mr10 fon-bold" href="./profile.html"></a>
            <p id="welcome" class="fon-15 mt20 mr20"></p>
            <button id="login-logout-button" class="fon-15 mt20">로그인</button>
        </div>
    </div>
</header>

<nav>
    <ul class="allMenu">
        <li><a href="./boardRecruitment.html?board=1" class="menu pppp30">그룹 스터디 모집</a></li>
        <li><a href="/qnaboard" class="menu pppp30">질문</a></li>
        <li><a href="./ProofShot.html?board=3" class="menu pppp30">인증</a></li>
        <li><a href="./ReviewBoard.html?board=4" class="menu pppp30">수강후기</a></li>
    </ul>
</nav>