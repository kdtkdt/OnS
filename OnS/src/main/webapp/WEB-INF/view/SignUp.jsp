	...
	<div id="wrapper" class="wid-100p">
		<div class="loginCon wid-100p">
			<div class="loginBox SignupBox">
				...
				<div class="inputForm">
					<label for="id" class="mb5">이메일</label>
					<input id="id" type="text" placeholder="이메일을 입력하세요" class="inpw wid-100p">
				</div>
				<div class="inputForm">
					<label for="nick" class="mb5">닉네임</label>
					<input id="nick" type="text" placeholder="닉네임을 입력하세요" class="inpw wid-100p">
				</div>				<div class="inputForm">
					<label for="pw" class="mb5">비밀번호</label>
					<input id="pw" type="password" placeholder="영문자/숫자/특수문자 조합 8~25자리" class="inpw wid-100p">
				</div>
				<input type="submit" value="Sign Up" id="signUp" class="btnLogin" onclick="joinform_check();">
				<div class="signUp">
					<span class="mr5">Already have an account?</span>
					<a href="login">로그인</a>
				</div>
			</div>
		</div>
			    ...
			    return false; 
					...
					//이메일 형식이 알파벳+숫자@알파벳+숫자.알파벳+숫자 형식이 아닐경우	
						...
						return false;
							...
							localStorage.setItem('username', $('.inpw:eq(1)').val()); // 시연용
					  		location.href = '/login';
				  			}
};
</script>
